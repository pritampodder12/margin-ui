...
import type { Project } from '@/store/resumeTypes';
...
export const ProjectCard = ({
  ...
  const deleteBullet = (i: number) => {
    onUpdate({ description: project.description.filter((_, idx) => idx !== i) });
  };
      ...
      >
        <div className="min-w-0 flex-1">
          ...
          <div className="text-[0.78rem] text-[var(--ink-faint)] mt-0.5 truncate">
            {[project.technologies, dateRange].filter(Boolean).join(' · ') || 'Technologies used'}
          </div>
        </div>
        {expanded ? (
